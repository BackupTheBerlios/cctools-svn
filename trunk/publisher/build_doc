...
from zope.interface.interface import InterfaceClass, Method, Attribute
import types
...
from epydoc.html import HTMLFormatter, _write_if_nonempty
...
class TwistedHTMLFormatter(HTMLFormatter):
    ...
    def _write_func_details(self, public, private, container, functions,
        ...
        for function in functions:
            ...
            private.write("".join([n for n in str if ord(n) < 128]))
...
### NRY 24 Jun 2005
# make attribute introspection work
_lookup_class_field_orig = objdoc._lookup_class_field

def _lookup_class_field(cls, field, base_order=None):
    result = _lookup_class_field_orig(cls, field, base_order)

    if result[0] ==  '<Not Accessible>':
        if isinstance(cls, InterfaceClass):
            target = cls[field]
            repr = result[0]
            
            if isinstance(target, (Attribute, Method)):
                repr = target.getDoc()
                
            result = (repr, result[1])

    return result

    pass
objdoc._lookup_class_field = _lookup_class_field
...
names = ['p6/', 'zope', 'ccwx', 'ccwsclient', 'pyarchive', ] #default, may be overriden below
...
sys.argv[1:1] = ['-n', 'P6 %s' % p6.VERSION, '-u', 'http://wiki.creativecommons.org/wiki/Developer']
